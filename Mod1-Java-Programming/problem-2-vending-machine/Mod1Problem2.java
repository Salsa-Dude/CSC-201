 ...
 * Display payment option list
 * While option is NOT equal to 0, keep displaying payment option list
 * - If user chooses option 1, display coin option list
 * - While option is NOT equal to 0, keep displaying coin option list
 * 	- Add the value of the coin into the total cost.
 * - If user chooses option 2, display bill option list
 * - While option is NOT equal to 0, keep displaying bill option list
 * 	- Add the value of the bill into the total cost
 * - If user chooses option 0, display product selection list.
 * - Subtract product price from total cost.
 * - Give user the change amount by first subtracting the total by $0.25.
 *  - Keep subtracting by $0.25 until no more
 * - Do the same process for $0.10, $0.05, and $0.01
 * - Print the coin name for each iteration.
...
public class Mod1Problem2 {
	...
	public static void test(int paymentInput, int coinInput, int billInput) {
		int total = 0;
		
		printPaymentMethod();
		
		int option = paymentInput;
		...
		while(option != 0) {
			...
			if(option == 1) {
				...
				while(coinFlag == true) {
					...
					printCoinList();
					Scanner coinMoreInput = new Scanner(System.in);
					 coinInput = coinMoreInput.nextInt();
				}
			}
				...
				printBillList();
				Scanner moreBillInput = new Scanner(System.in);
				billInput = moreBillInput.nextInt();
			}
